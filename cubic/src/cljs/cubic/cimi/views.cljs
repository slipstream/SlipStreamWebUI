(ns
  ...
  (:require
    ...
    [cubic.editor.editor :as editor]
    [cubic.utils.component :as cutil]
    [cubic.utils.forms :as form-utils]
    [cubic.utils.semantic-ui :as ui]

    [cubic.cimi.subs :as cimi-subs]
    [cubic.cimi.events :as cimi-events]
    [cubic.cimi-detail.views :as cimi-detail-views]
...
(defn results-table [selected-fields entries]
  (when (pos? (count entries))
    (let [row-fn (results-table-row-fn selected-fields)]
      [ui/Container {:class-name "cubic-x-autoscroll"}
       [ui/Table
        {:collapsing  true
         :compact     true
         :single-line true
    ...
    (fn []
      (let [resource-key (get (:collection-key @cloud-entry-point) @collection-name)
        ...
        (when (and tpl-resource-key (empty? @descriptions-vector-atom))
          ...
          (dispatch [:evt.cubic.cimi/get-templates tpl-resource-key])) ;; FIXME
        ...
        (when @show?
          (cond
            ...
            [form-utils/credential-form-container-modal
               ...
               :on-submit (fn [data]
                            (dispatch [:evt.cubic.cimi/create-resource resource-key data]) ;; FIXME
                 ...
                 {:on-click (fn []
                              (try
                                (let [data (js->clj (.parse js/JSON @text))]
                                  (dispatch [:evt.cubic.cimi/create-resource-no-tpl resource-key data])) ;; FIXME
                                (catch js/Error e
